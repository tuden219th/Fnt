...
import { Card, CardContent, CardDescription } from '@open-mercato/ui/primitives/card'
...
import type { AuthOverride, LoginFormWidgetContext } from '@open-mercato/core/modules/auth/frontend/login-injection'
...
export default function FntLoginPage() {
  const t = useT()
  ...
  return (
    <div className="min-h-svh bg-[radial-gradient(circle_at_top,_#fff7ed_0%,_#fff_30%,_#f8fafc_100%)] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="mb-5 flex flex-col items-center justify-center gap-2 text-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-xl border border-orange-200 bg-orange-50 text-xl font-black tracking-tight text-orange-600 shadow-sm ring-1 ring-orange-100">
            F
          </div>
          <div className="text-[1.9rem] font-black tracking-[-0.06em] text-slate-900">FnT ErrP</div>
          <CardDescription className="text-sm text-slate-500">
            {translate('auth.login.subtitle', 'Access your workspace')}
          </CardDescription>
        </div>

        <Card className="w-full border border-slate-200 bg-white/90 shadow-[0_18px_40px_rgba(15,23,42,0.08)] backdrop-blur-sm">
          <CardContent className="p-5 sm:p-6">
            <LoginFormSection>
              <form className="grid gap-3" onSubmit={onSubmit} noValidate data-auth-ready={formReady ? '1' : '0'}>
                ...
                {activeAuthenticatedUser && (translatedRoles.length || translatedFeatures.length) ? (
                  <div className="flex justify-center" data-testid="login-return-dashboard">
                    <Button asChild type="button" variant="outline" size="sm">
                      <Link href="/backend">{translate('auth.accessDenied.dashboard', 'Go to Dashboard')}</Link>
                    </Button>
                  </div>
                ) : null}
                {showTenantInvalid ? (
                ...
                <Button type="submit" disabled={submitting || !formReady} className="mt-2 h-10 bg-orange-600 text-white hover:bg-orange-700 focus-visible:ring-orange-500">
                  {submitting
